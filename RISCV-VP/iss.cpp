	...
	*/
	...
	switch (op) {
				...
				case 0b111: {
					switch (instr.funct7()) {
						...
						case 0b0000001: {
							...
							if (sm3_nice.cycles == 6)
								printf("custom3_sm3p0 timing error ! \n");
							pipeline.advance(sm3_nice.cycles-1);
							#else
						...
						case 0b0000010: {
							...
							if (sm3_nice.cycles == 6)
								printf("custom3_sm3p1 timing error ! \n");
							pipeline.advance(sm3_nice.cycles-1);
							#else
						...
						case 0b0000011: {
							...
							if (sm4_nice.cycles == 14)
								printf("custom3_sm4ks timing error ! \n");
							pipeline.advance(sm4_nice.cycles-1);
							#else
						...
						case 0b0000100: {
							...
							if (sm4_nice.cycles == 14)
								printf("custom3_sm4ed timing error ! \n");
							pipeline.advance(sm4_nice.cycles-1);
							#else
		...
		case Opcode::SB: {
			...
			#ifdef E203_TIMING
			pipeline.reserve_store({addr, addr+1}, 1);
			#else
		...
		case Opcode::SH: {
			...
			#ifdef E203_TIMING
			pipeline.reserve_store({addr, addr+2}, 1);
			#else
		...
		case Opcode::SW: {
			...
			#ifdef E203_TIMING
			pipeline.reserve_store({addr, addr+4}, 1);
			#else
		...
		case Opcode::LB: {
			...
			#ifdef E203_TIMING
			pipeline.do_load({addr, addr+1}, instr.rd(), 2);
			#else
		...
		case Opcode::LH: {
			...
			#ifdef E203_TIMING
			pipeline.do_load({addr, addr+2}, instr.rd(), 2);
			#else
