module uart_rx
#(
    parameter CLK_FRE     = 100,              //时钟频率，用于计数，默认100MHz，最好与系统时钟频率i_clk_sys一致
    ...
    parameter BAUD_RATE   = 115200            //波特率，缺省为115200
)
(
    ...
    //UART输入是异步输入，最好是同步到FPGA内部的时钟域，可以省略，但在异步电路中，保持时钟域同步是一个良好习惯
    reg sync_uart_rx;
    ...
    //连续采样五个接收路电平来判断rx是否有信号传来，用五个采样信号来作为判断标准可以有效排除毛刺噪声带来的误判
    ...
    always@(posedge i_clk_sys or negedge i_rst_n)
        begin
            if(!i_rst_n)
                ...
                case(r_current_state)
                    STATE_IDLE:begin //闲置状态下对寄存器进行复位                          
                            r_rcv_cnt <= 4'd0;
                            r_data_rcv <= 'd0;
                            r_parity_check <= 1'b0;
                            o_rx_done <= 1'b0;
                            if(r_flag_rcv_start == 5'b00000) //连续检测到低电平时认为UART传来数据，拉高baud_valid
                                baud_valid <= 1'b1;
                        end
                    STATE_START:begin
                    ...
                    STATE_PARITY:begin
                            if(baud_pulse)
                                begin
                                    if(r_parity_check + sync_uart_rx == PARITY_TYPE)//校验检测，正确则o_ld_parity拉高，可输出给led检测，如果闪烁则表示有错误数据发生
                                        o_ld_parity <= 1'b1;
                                    else
                                        o_ld_parity <= 1'b0;
                                end
                    ...
                    STATE_END:begin
                            if(baud_pulse)
                                begin
                                    if(PARITY_ON == 0 || o_ld_parity)//没有校验位或者校验位正确时才输出数据，否则直接丢弃数据
                                        begin
